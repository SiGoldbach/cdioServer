...
def imageRecognitionHD(image):
    ...
    if detected_Balls is not None:
        ...
        for pt in detected_circles[0, :]:
            ...
            if np.logical_and.reduce((50 >= image[b, a][0], 160 <= image[b, a][1], 200 >= image[b, a][1], 160 <= image[b, a][2])):
    ...
    if detected_Robot is not None:
        ...
        for pt in detected_circles[0, :]:
            a, b, r = pt[0], pt[1], pt[2]
            if np.logical_and.reduce((220 >= image[b, a][0], 70 <= image[b, a][0], 200 <= image[b, a][1], 140 <= image[b, a][2])):
                ...
                circle += 1
                continue
            if np.logical_and.reduce((150 <= image[b, a][0], 140 <= image[b, a][1], 255 >= image[b, a][1], 120 <= image[b, a][2], 220 >= image[b, a][2])):
    ...
    for contour in cnts:
        ...
        if image[y, x][2] > 120:
            ...
            if M["m00"] != 0:
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                if np.logical_or(w > 500, np.logical_and(50 < w, w < 100)):
    ...
    print(obstacle)
    print("Amount of red pixels: " + str(len(obstacle)))
    ...
    cv.waitKey(0)
    return front, back, balls, smallGoal, obstacle
